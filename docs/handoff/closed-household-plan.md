...
- [ ] Follow this dependency graph. Start dependent work only after its parent merges, or base it on the parent branch when the execution playbook stacks.
    ...
    - [ ] PR-3 after PR-2.
    - [ ] PR-4 after PR-3.
- [ ] Hold the file boundaries. PR-1 touches login, `src/oauth.ts`, `src/dashboard.ts` create-household, `src/supabase.ts` create helper, `src/household.ts` create helper, a new migration, and the tests for those. PR-2 touches `src/mcp.ts` household admin tools, owner checks in `src/household.ts`, `add_household_member` SQL, `README.md`, and tests. PR-3 touches the site dashboard add-member form and its tests. PR-4 touches bootstrap SQL, grants, and deletion of unused join helpers.
...
- [ ] The root appends the PR to the base-branch stack. The operator lands it bottom-up. Patch-id must match the verdict SHA per `playbooks/shipping.md`.
...
- [ ] Edit `src/oauth.ts`. Allow `signUpUser` only when Auth has zero users. After the first user exists, `/approve` signs in and does not sign up. Do not add a join helper. Trunk has none.
...
- [ ] Edit `src/household.ts`. Add `createHousehold` that inserts the singleton household and the caller as owner in one step, and fails if a household already exists.
- [ ] Edit `src/supabase.ts`. Add a service-role call that creates the household plus the owner in one transaction. Do not insert a member when a household already exists. Do not add `ensureHouseholdMembership`.
- [ ] Create `supabase/migrations/YYYYMMDDHHMMSS_create_household.sql` with `supabase migration new create_household`. The function takes the caller user id, household name, and display name. It runs in one transaction.
...
- [ ] Do not add join-as-member on login or on `GET /`. Trunk has none. Discard any local uncommitted `ensureHouseholdMembership` patch.
- [ ] Add `create_household` RPC and the site form that calls it for the first Auth user.
- [ ] Reject public `signUpUser` once any Auth user exists. Sign-in of existing users stays.
...
- [ ] `src/oauth.test.ts` gains a case that a second signup is refused.
- [ ] `src/household.test.ts` gains a case that a second `createHousehold` fails.
- [ ] `src/dashboard.test.ts` or `src/mcp.test.ts` site describe gains a case that a non-member sees 403 when a household already exists.
- [ ] `src/mcp.test.ts` does not add an auto-join mock as the spec. Run `bun test src/oauth.test.ts src/household.test.ts src/dashboard.test.ts src/mcp.test.ts src/public-site.test.ts`.
...
- [ ] Lane 9. `GET /health`. Save `pr1-l9-health.png`. Pass when the body is `ok`.
...
- [ ] Edit `supabase/migrations/YYYYMMDDHHMMSS_owner_admin.sql`. Keep `add_household_member` as role member. Document that caller checks live in MCP because the RPC is service-role only.
...
- [ ] `src/mcp.test.ts` changes the "any member may rotate" case to owner-only, and adds a member-refused case for `add_household_member`. Run `bun test src/household.test.ts src/mcp.test.ts`.

**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes on `grok-4.6-fast-xhigh` at the PR head, per the boot recipe.

- [ ] Lane 1. Regression lane against trunk. Call `rotate_household_token` as a member at trunk and at head. If trunk allows it, record that. Gate that head refuses. Save `pr2-l1-regression.png`. Pass when head is refused.
...
- [ ] Edit `src/dashboard.test.ts` and `src/mcp.test.ts` site describe.
...
- [ ] Site HTTP tests post the form as owner and as member. Run `bun test src/dashboard.test.ts src/mcp.test.ts src/index.test.ts`.
...
- [ ] Create `supabase/migrations/YYYYMMDDHHMMSS_drop_bootstrap_join.sql`. Replace `bootstrap_household` so a second caller gets an error instead of a member row. `REVOKE` execute from `authenticated`.
...
No prototype branch. Product calls were settled in chat. Unproven items are the exact create-household field copy and whether Google stays on the login page after signup closes.
...
This repo has no `pstack/` tree. Tick re-reads use the plugin cache. Record that each tick.
