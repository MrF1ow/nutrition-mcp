...
**Registered tool set.** `src/mcp.ts` has 42 `server.registerTool()` calls. `src/mcp.test.ts` asserts `TOOLS` names in `src/copy/tools.ts` match `tools/list` and that person-scoped catalog cards include optional `user_id`. That file is an identity catalog (names, params, household/oauth splits), not a public `/tools` page.
...
The only public HTML is the OAuth login template: `public/login.html` and `public/{locale}/login.html`, written by `scripts/gen-login.ts`. Visitors see it only through `GET /authorize` with a valid OAuth query. `GET /` is 404. Do not add marketing routes, and do not restore `gen-index.ts`, `gen-tools.ts`, `gen-legal.ts`, `gen-alternatives.ts`, or `gen-sitemap.ts`.
...
`scripts/site-partials.ts` `nav()` / `footer()` are login-only: brand (not a link to `/`), language switcher token, theme. No tools, install, legal, or alternatives links. Consent `{terms}` / `{privacy}` render as localized text, not anchors to `/terms` or `/privacy`.

Shared assets still served: `/styles.css`, `/site.js`, `/favicon.ico`. `site.js` owns the theme toggle. It must not poll `/api/stats`. Keep the pre-paint theme script right after `<body>` so dark-mode visitors do not flash light.

`src/copy/login.ts` plus its locale files hold the copy. `src/copy/chrome.ts` still supplies labels. `src/copy/tools.ts` `TOOLS` is the MCP catalog for `mcp.test.ts`, not a page.

### Locales

German, Spanish, French, Dutch, Polish, Italian, Ukrainian, Japanese (`LOCALES` in `src/routes.ts`). Login locale is `?locale=` on `/authorize`, not a path prefix. Availability is file existence (`availableLoginLocales()` in `src/oauth.ts`). `LOGIN` is a full `Record<SiteLocale, LoginDoc>`, so typecheck fails if a locale is added without copy.

Re-run `bun run gen:all` after editing login copy or `site-partials.ts`. The generated file is still a template: `{{SESSION_ID}}`, `{{ERROR}}`, `{{LANG_SWITCHER}}`, `{{TRANSLATION_NOTICE}}` are filled per request. Switcher links rebuild `/authorize?...` from the in-flight session.

`scripts/depersonalize.ts` strips GA, the Glama route, and the widget support email. It discovers login templates at run time and must not assume index, tools, legal, or alternatives files exist.
